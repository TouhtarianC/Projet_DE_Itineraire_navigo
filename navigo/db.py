...
def get_poi_by_zone(zone: int, rayon: int, days: int = 1) -> list:
    ...
    while iteration < MAX_LOOKUP_ITERATIONS_FOR_POINTS and \
        ...
        l_postal_code = get_nearby_communes_as_where_clause(zone, rayon)[1:-1].split(',')
        # logger.info(f"l_postal_code = {l_postal_code}")
        try:
            poi_list = session.query(Poi).filter(
                Poi.POSTAL_CODE.in_(l_postal_code)).all()
    ...
    # limitation of lenght of POI list
    if len(poi_list) > min_nb_POI*3:
        poi_list = poi_list[:min_nb_POI*3]
    ...
    while iteration < MAX_LOOKUP_ITERATIONS_FOR_POINTS and \
        ...
        try:
            trail_list = session.query(Trail).filter(
                Trail.POSTAL_CODE.in_(l_postal_code)).all()
        except Exception as e:
